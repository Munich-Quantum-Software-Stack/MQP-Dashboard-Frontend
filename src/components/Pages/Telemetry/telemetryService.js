...
export function getAvailableCategories(environmentSensors) {
  ...
  return order.filter((key) => {
    ...
    return (environmentSensors[key]?.length || 0) > 0; // eslint-disable-line security/detect-object-injection
  });
}
...
export function findPanelRefForCategory(environmentSensors, category) {
  ...
  if (category === 'temperature') {
    ...
    sensors = environmentSensors[category] || []; // eslint-disable-line security/detect-object-injection
  }
...
function assertTelemetryApiUrl() {
...
export async function getRoomData(roomId) {
  const { TELEMETRY_API_URL } = getConfig();
  const mock = MOCK_ROOMS[roomId]; // eslint-disable-line security/detect-object-injection
  ...
  if (!TELEMETRY_API_URL) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      const rows = generateDummyTimeseries(sensorIds[i], from, to, 10).map(
        ({ timestamp, value }) => `${sensorIds[i]},${timestamp},${value}`, // eslint-disable-line security/detect-object-injection
      );
      allRows.push(...rows);
 ...
 * @returns {Promise<Array<{name: string, sensors: string[]}>>}
 */
export async function getTelemetrySensors() {
  ...
  const telemetryApiUrl = assertTelemetryApiUrl();
 ...
 * @param {{name:string, sensors:string[]}[]} measurements
